...
public class IniParser {
  ...
  private HashMap<String, HashMap<String, String>> state;
  // @formatter:on
  ...
  private static Pattern composedPattern = Pattern.compile(
    String.format("^(%s)|(%s)$", linePattern, sectionPattern)
  );
  ...
  public static IniParser fromFile(String filePath) throws IOException {
    ...
    // @formatter:off
    parser.state = Files.lines(file.toPath()).map(
      str -> composedPattern.matcher(str).results().map(
        r -> // compute dumb null throwable regex >_>
          Try.of(
            () -> Either.either(
    ...
    ).filter(r -> r != null).reduce(
      // counter, map
      Tuple.of("", HashMap.of("", HashMap.of("", ""))),
      (state, item) -> {
        ...
        return item.fold(
          line -> Tuple.of(
            ...
            sections.put(
              parent,
              sections.get(parent).getOrNull().put(line.get(0), line.get(1))
            )
          ),
          section -> Tuple.of(
            ...
            sections.put(section, HashMap.empty())
          )
        );
      },
      (a, b) -> a
  ...
  public <T> T get(String section, String key) {
    return (T) state.getOrElse(section, HashMap.empty()).getOrElse(key, "");
  }
